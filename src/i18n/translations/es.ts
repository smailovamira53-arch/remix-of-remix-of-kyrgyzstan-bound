export const es = {
  // Navbar
  nav: {
    home: 'Inicio',
    tours: 'Tours',
    tourPackages: 'Paquetes turísticos',
    dayExcursions: 'Excursiones de un día',
    multiDayTours: 'Tours de varios días',
    adventure: 'Expediciones',
    accommodations: 'Alojamiento',
    aboutKyrgyzstan: 'Sobre Kirguistán',
    travelTips: 'Consejos de Viaje',
    contact: 'Contacto',
    guides: 'Guías',
    signIn: 'Iniciar sesión',
    bookNow: 'Reservar ahora',
    exploreMore: 'Descubrir más',
  },

  // Hero
  hero: {
    badge: 'Descubre el corazón de Asia Central',
    title: 'Experimenta la magia de',
    titleHighlight: 'Kirguistán',
    description: 'Montañas impresionantes, lagos alpinos prístinos, antigua herencia de la Ruta de la Seda y cálida hospitalidad nómada te esperan en esta joya de Asia Central.',
    exploreTours: 'Explorar tours',
    planTrip: 'Planificar viaje',
    whereTo: '¿A dónde?',
    allDestinations: 'Todos los destinos',
    when: '¿Cuándo?',
    selectDates: 'Seleccionar fechas',
    travelers: 'Viajeros',
    adults: '2 Adultos',
    searchTours: 'Buscar tours',
    stats: {
      travelers: 'Viajeros felices',
      tours: 'Tours únicos',
      rating: 'Calificación promedio',
      experience: 'Años de experiencia',
    },
  },

  // Popular Tours
  popularTours: {
    subtitle: 'Tours populares',
    title: 'Aventuras inolvidables te esperan',
    description: 'Descubre nuestros tours más queridos, seleccionados por viajeros de todo el mundo por sus experiencias únicas y paisajes impresionantes.',
    viewAll: 'Ver todos los tours',
  },

  // Tour Card
  tourCard: {
    featured: 'Destacado',
    maxPeople: 'Máx. {count} personas',
    from: 'Desde',
    perPerson: '/persona',
    viewDetails: 'Ver detalles',
    priceOnRequest: 'Precio a consultar',
    countries: '89+ países',
  },

  // Tours Data
  tours: {
    issykKulAdventure: {
      title: 'Aventura de 7 días en el lago Issyk-Kul',
      location: 'Región de Issyk-Kul',
      duration: '7 días / 6 noches',
    },
    horseTrekking: {
      title: 'Cabalgata por el valle de Jeti-Oguz',
      location: 'Karakol, Issyk-Kul',
      duration: '3 días / 2 noches',
    },
    silkRoad: {
      title: 'Explorador de 10 días por la Ruta de la Seda',
      location: 'Múltiples regiones',
      duration: '10 días / 9 noches',
    },
    alaArcha: {
      title: 'Caminata de un día en Ala-Archa',
      location: 'Cerca de Biskek',
      duration: '1 día',
    },
    nomadGames2026: {
      title: 'Juegos Mundiales Nómadas 2026 — Cultura nómada épica y competiciones',
      location: 'Biskek – Issyk-Kul',
      duration: '10 días / 9 noches',
    },
  },

  // Excursions
  excursions: {
    subtitle: 'Excursiones de un día',
    title: 'Escapadas perfectas desde cualquier base',
    description: '¿Poco tiempo? Estas excursiones cuidadosamente seleccionadas ofrecen experiencias increíbles en pocas horas.',
    burana: {
      title: 'Torre Burana e historia de la Ruta de la Seda',
      description: 'Explora el antiguo minarete de Burana y petroglifos de la era de la Ruta de la Seda.',
      duration: 'Medio día',
    },
    jetiOguz: {
      title: 'Tour de un día a los Siete Toros de Jeti-Oguz',
      description: 'Admira las famosas formaciones rocosas rojas y disfruta de un almuerzo kirguís tradicional.',
      duration: 'Día completo',
    },
    songKul: {
      title: 'Experiencia nocturna en el lago Song-Kul',
      description: 'Duerme en una yurta tradicional junto al lago alpino a 3.000 m de altitud.',
      duration: '2 días',
    },
  },

  // Adventure Pass
  adventurePass: {
    badge: 'Mejor valor',
    title: 'Pase de aventura de Kirguistán',
    description: 'Desbloquea acceso ilimitado a las mejores atracciones, tours y experiencias de Kirguistán con nuestro pase todo incluido. Perfecto para exploradores curiosos.',
    benefits: [
      'Acceso ilimitado a más de 30 atracciones',
      'Traslados gratuitos desde el aeropuerto',
      'Reserva prioritaria para tours populares',
      'Descuentos exclusivos en hoteles asociados',
      'Soporte de viaje 24/7',
      'Guía digital con mapas sin conexión',
    ],
    getPass: 'Obtener pase',
    startingFrom: 'Desde',
    days: 'días',
    mostPopular: 'Más popular',
    purchaseNow: 'Comprar ahora',
    cancellation: 'Cancelación gratuita hasta 48 horas antes de la activación',
  },

  // Activities
  activities: {
    subtitle: 'Actividades de aventura',
    title: 'Encuentra tu aventura perfecta',
    description: 'Desde actividades llenas de adrenalina hasta experiencias culturales tranquilas, Kirguistán ofrece algo para cada tipo de viajero.',
    toursCount: '{count} tours',
    trekking: {
      title: 'Senderismo',
      description: 'Explora senderos de alta montaña a través de paisajes prístinos.',
    },
    horseRiding: {
      title: 'Paseos a caballo',
      description: 'Experimenta las tradiciones nómadas a caballo por praderas alpinas.',
    },
    paragliding: {
      title: 'Esquí de travesía',
      description: 'Aventuras de esquí invernal en las montañas de Kirguistán.',
    },
    yurtCamping: {
      title: 'Camping en yurta',
      description: 'Duerme en viviendas nómadas tradicionales bajo cielos estrellados.',
    },
    photography: {
      title: 'Tours fotográficos',
      description: 'Captura paisajes increíbles con guía experto.',
    },
    biking: {
      title: 'Ciclismo de montaña',
      description: 'Recorre terrenos diversos desde valles hasta pasos de montaña.',
    },
  },

  // Testimonials
  testimonials: {
    subtitle: 'Opiniones de viajeros',
    title: 'Lo que dicen nuestros huéspedes',
    description: 'Historias reales de aventureros que descubrieron la magia de Kirguistán con nosotros.',
    items: [
      { name: 'Sarah Mitchell', location: 'Estados Unidos', text: 'La noche en Song-Kul fue absolutamente mágica. Despertar en una yurta con caballos pastando afuera y el amanecer sobre el lago — un recuerdo que atesoraré siempre.', tour: 'Explorador de 10 días por la Ruta de la Seda' },
      { name: 'Thomas Bergmann', location: 'Alemania', text: 'Tour increíblemente bien organizado con guías muy conocedores. La cabalgata por Jeti-Oguz superó todas las expectativas. ¡Los paisajes son sencillamente de otro mundo!', tour: 'Aventura a caballo' },
      { name: 'Yuki Tanaka', location: 'Japón', text: 'Kirguistán no estaba en mis planes hasta que un amigo me recomendó este tour. Ahora es mi destino favorito. La hospitalidad y la belleza natural son incomparables.', tour: 'Aventura de 7 días en Issyk-Kul' },
    ],
  },

  // Footer
  footer: {
    newsletter: {
      title: 'Inspírate para viajar',
      description: 'Suscríbete para ofertas exclusivas, consejos de viaje e información sobre Kirguistán.',
      placeholder: 'Ingresa tu email',
      subscribe: 'Suscribirse',
    },
    brand: {
      description: 'Tu puerta a aventuras auténticas en el corazón de Asia Central. Descubre los paisajes impresionantes y la rica cultura nómada de Kirguistán.',
    },
    explore: 'Explorar',
    destinations: 'Destinos',
    company: 'Empresa',
    support: 'Soporte',
    links: {
      tourPackages: 'Paquetes turísticos',
      dayExcursions: 'Excursiones de un día',
      multiDayTours: 'Tours de varios días',
      adventureActivities: 'Actividades de aventura',
      adventurePass: 'Pase de aventura',
      issykKul: 'Lago Issyk-Kul',
      songKul: 'Lago Song-Kul',
      alaArcha: 'Garganta de Ala-Archa',
      jetiOguz: 'Jeti-Oguz',
      bishkek: 'Biskek',
      aboutUs: 'Sobre nosotros',
      ourTeam: 'Nuestro equipo',
      travelBlog: 'Blog de viajes',
      careers: 'Carreras',
      contact: 'Contacto',
      faqs: 'Preguntas frecuentes',
      bookingTerms: 'Términos de reserva',
      privacyPolicy: 'Política de privacidad',
      travelInsurance: 'Seguro de viaje',
      visaInfo: 'Información de visa',
      travelTips: 'Consejos de Viaje',
    },
    copyright: '© 2025 Mountain Magic Tours. Todos los derechos reservados.',
  },

  // Common
  common: {
    perPerson: '/persona',
    days: 'días',
    day: 'día',
    nights: 'noches',
    from: 'Desde',
    bookNow: 'Reservar ahora',
    viewDetails: 'Ver detalles',
    allTours: 'Todos los tours',
    filterBy: 'Filtrar por',
    duration: 'Duración',
    category: 'Categoría',
    priceRange: 'Rango de precio',
    search: 'Buscar',
    noResults: 'No se encontraron tours',
    clearFilters: 'Limpiar filtros',
    perNight: '/noche',
    details: 'Detalles',
    max: 'Máx.',
  },

  // Travel Tips Page
  travelTips: {
    title: 'Consejos de Viaje y FAQ',
    subtitle: 'Información Esencial para Turistas',
    description: 'Todo lo que necesitas saber antes de visitar Kirguistán. Consejos prácticos, costos e información privilegiada.',
    
    visa: {
      title: 'Visa y Entrada',
      items: [
        'Acceso sin visa por 60 días para más de 60 países (UE, EE.UU., Reino Unido, Australia, etc.)',
        'E-visa disponible en línea por $50–100 para otras nacionalidades',
        'Permisos fronterizos requeridos para áreas restringidas cerca de Tayikistán, China o Kazajistán ($30–50 vía operador turístico)',
        'Sin vacunas obligatorias, pero mantén las rutinarias actualizadas (Hepatitis A, Tétanos)',
      ],
    },
    
    safety: {
      title: 'Seguridad',
      items: [
        'Generalmente seguro para turistas con bajas tasas de criminalidad',
        'Ten precaución en Bishkek de noche, especialmente cerca de bazares concurridos como Osh Bazaar',
        'Bajo riesgo de terrorismo, pero mantente informado sobre alertas para regiones del sur',
        'Evita mostrar grandes cantidades de efectivo; usa taxis oficiales o transporte prearreglado',
        'Mujeres viajeras: viste modestamente en áreas rurales; viajar sola es seguro pero únete a grupos para caminatas remotas',
      ],
    },
    
    transport: {
      title: 'Transporte',
      items: [
        'Aeropuerto Manas: Usa el mostrador oficial de taxis (~1000 som al centro de Bishkek)',
        'Marshrutkas (minibuses): 400–500 som de Bishkek a Karakol (5-6 horas)',
        'Taxis compartidos: Más rápidos, negocia precios (500–700 som por persona)',
        'Vuelos domésticos: Tez Jet ofrece Bishkek–Osh por ~$30–50 (1 hora)',
        'Alquiler de 4x4 recomendado para carreteras de montaña',
        'Autoestop: Común y generalmente seguro en áreas rurales',
      ],
    },
    
    currency: {
      title: 'Moneda',
      items: [
        'Moneda local: Som kirguís (KGS); 1 USD ≈ 85–90 KGS',
        'Cambia USD o Euros en bancos o casas de cambio oficiales en Bishkek/Osh',
        'Cajeros automáticos ampliamente disponibles en ciudades (dispensan KGS o USD)',
        'Efectivo esencial para áreas rurales y mercados',
        'Tarjetas aceptadas solo en hoteles y restaurantes de ciudades',
        'Propina: 10% en restaurantes, redondea para taxis',
      ],
    },
    
    culture: {
      title: 'Cultura y Etiqueta',
      items: [
        'Los kirguises son muy hospitalarios – acepta el té y la comida con gracia',
        'Quítate los zapatos al entrar a las casas',
        'El ruso básico es útil; el inglés es limitado fuera de zonas turísticas',
        'Debes probar: Beshbarmak (carne y fideos), Plov (arroz), Kumis (leche de yegua fermentada)',
        'Respeta las tradiciones nómadas y a los mayores',
      ],
    },
    
    health: {
      title: 'Salud y Equipaje',
      items: [
        'El mal de altura es común en Song-Kul (3000m) – aclimátate lentamente y mantente hidratado',
        'Bebe solo agua embotellada o hervida; evita el agua del grifo',
        'Clima: Veranos calurosos (hasta 35°C), inviernos fríos (-20°C); empaca capas para montañas',
        'Esenciales: Protector solar, repelente de insectos, zapatos cómodos para caminar',
        'Viste modestamente al visitar mezquitas o aldeas rurales',
      ],
    },
  },

  // Contact Page
  contact: {
    title: 'Contáctenos',
    subtitle: 'Ponte en Contacto',
    description: '¿Tienes preguntas sobre nuestros tours? ¡Estamos aquí para ayudar!',
    info: {
      title: 'Información de Contacto',
      address: '123 Avenida Chui, Biskek 720001, Kirguistán',
      email: 'info@mountainmagictours.com',
      phone: '+996 555 123 456',
      hours: 'Lun-Vie: 9:00 AM - 6:00 PM (Hora de Biskek)',
    },
    form: {
      title: 'Envíanos un Mensaje',
      name: 'Tu Nombre',
      email: 'Tu Email',
      subject: 'Asunto',
      message: 'Tu Mensaje',
      submit: 'Enviar Mensaje',
      success: '¡Gracias! Tu mensaje ha sido enviado.',
      subjects: {
        general: 'Consulta General',
        booking: 'Pregunta de Reserva',
        customTour: 'Tour Personalizado',
        feedback: 'Comentarios',
        partnership: 'Oportunidad de Asociación',
      },
    },
    social: {
      title: 'Síguenos',
      facebook: 'Facebook',
      instagram: 'Instagram',
      twitter: 'Twitter',
    },
  },

  // Tours Page
  toursPage: {
    title: 'Nuestros Tours',
    subtitle: 'Explora Kirguistán',
    description: 'Desde excursiones de un día hasta aventuras de varias semanas.',
    backToHome: 'Volver al inicio',
    exploreTitle: 'Explora nuestros tours',
    toursFound: 'tours encontrados',
    filtersToggle: 'Filtros',
    clearAll: 'Limpiar',
    noResults: 'No hay tours que coincidan con tus filtros.',
    clearAllFilters: 'Limpiar todos los filtros',
    categories: {
      all: 'Todos los Tours',
      multiDay: 'Tours Multiday',
      horseback: 'Paseos a Caballo',
      trekking: 'Senderismo',
      cultural: 'Tours Culturales',
      family: 'Tours Familiares',
      winter: 'Tours de Invierno',
      biking: 'Tours en Bicicleta',
      offroad: 'Aventuras Todoterreno',
      packages: 'Paquetes turísticos',
      dayExcursions: 'Excursiones de un día',
    },
    filters: {
      duration: 'Duración',
      category: 'Categoría',
      price: 'Rango de Precio',
      region: 'Región',
      activityType: 'Tipo de actividad',
    },
    activities: {
      trekking: 'Senderismo',
      horseRiding: 'Paseos a caballo',
      skiTouring: 'Esquí de travesía',
      yurtCamping: 'Camping en yurta',
      photography: 'Tours fotográficos',
      mountainBiking: 'Ciclismo de montaña',
    },
    durations: {
      short: '1-3 días',
      medium: '4-7 días',
      long: '8-14 días',
      extended: '15+ días',
      d13: '1–3 días',
      d47: '4–7 días',
      d8: '8+ días',
    },
    priceRanges: {
      under300: 'Menos de $300',
      mid: '$300–$600',
      over600: '$600+',
    },
  },

  // Guides
  guides: {
    subtitle: 'Nuestros Guías',
    title: 'Conoce a Nuestros Guías Expertos',
    description: 'Nuestros guías locales certificados aportan años de experiencia y pasión para hacer tu aventura inolvidable.',
    pageTitle: 'Nuestros Guías',
    pageDescription: 'Descubre nuestro equipo de guías locales certificados listos para mostrarte lo mejor de Kirguistán.',
    reviews: 'reseñas',
    perDay: 'día',
    bookGuide: 'Reservar Guía',
    askar: {
      name: 'Askar',
      specialties: ['Equitación', 'Senderismo'],
      bio: 'Guía ecuestre experto con 15 años de experiencia en montaña.',
    },
    aida: {
      name: 'Aida',
      specialties: ['Tours Culturales', 'Tours de Ciudad'],
      bio: 'Experta en Bishkek y Osh con profundo conocimiento cultural.',
    },
    nurlan: {
      name: 'Nurlan',
      specialties: ['Alpinismo', 'Pamir'],
      bio: 'Montañero certificado especializado en expediciones al Pamir.',
    },
    gulnara: {
      name: 'Gulnara',
      specialties: ['Fotografía', 'Naturaleza'],
      bio: 'Fotógrafa profesional guiando tours panorámicos.',
    },
    bakyt: {
      name: 'Bakyt',
      specialties: ['Estancias en Yurta', 'Vida Nómada'],
      bio: 'Experto en tradiciones nómadas y campamentos en yurta.',
    },
    aizhan: {
      name: 'Aizhan',
      specialties: ['Esquí', 'Tours de Invierno'],
      bio: 'Especialista en deportes de invierno e instructor de esquí.',
    },
    marat: {
      name: 'Marat',
      specialties: ['Todoterreno', 'Tours 4x4'],
      bio: 'Experto en todoterreno para destinos remotos de montaña.',
    },
    dinara: {
      name: 'Dinara',
      specialties: ['Gastronomía', 'Cocina'],
      bio: 'Guía culinaria ofreciendo experiencias gastronómicas tradicionales.',
    },
    timur: {
      name: 'Timur',
      specialties: ['Ciclismo', 'Bicicleta'],
      bio: 'Experto en ciclismo de montaña para todos los niveles.',
    },
    cholpon: {
      name: 'Cholpon',
      specialties: ['Tours Familiares', 'Niños'],
      bio: 'Especializada en aventuras familiares.',
    },
  },

  // New Tours
  newTours: {
    songKolKarakol: {
      title: 'Viaje Semanal a Song-Kol y Karakol',
      location: 'Song-Kol y Karakol',
      duration: '5 Días / 4 Noches',
      description: 'Explora lagos alpinos prístinos y sumérgete en la cultura kirguisa.',
    },
    almatyBishkek: {
      title: 'De Almaty a Bishkek',
      location: 'Kazajstán - Kirguistán',
      duration: '6 Días / 5 Noches',
      description: 'Aventura transfronteriza a través de impresionantes pasos de montaña y lagos.',
    },
    horsebackSongKol: {
      title: 'Aventura a Caballo de 3 Días a Song-Kol',
      location: 'Lago Song-Kol',
      duration: '3 Días / 2 Noches',
      description: 'Experimenta la equitación nómada tradicional hacia el lago alpino.',
    },
  },

  heroSlides: {
    slide1: 'Kirguistán: Siente el Aire de los Nómadas',
    slide2: 'Descubre la Antigua Cultura Nómada',
    slide3: 'Lagos Alpinos Prístinos y Aventuras',
    slide4: '¡Únete a los Juegos Mundiales Nómadas 2026 – Septiembre en Cholpon-Ata!',
    slide5: 'Épicos Trekkings y Viajes a Caballo',
  },

  nomadGames: {
    banner: '🏇 ¡No te pierdas los Juegos Mundiales Nómadas 2026 (septiembre)! Combina con tu tour – ¡cultura nómada épica y competiciones!',
    explore: 'Explorar',
  },

  // Tour Detail Page
  tourDetail: {
    notFound: 'Tour no encontrado',
    notFoundDescription: 'El tour que buscas no existe.',
    backToTours: 'Volver a Tours',
    back: 'Volver',
    highlights: 'Destacados del Tour',
    itinerary: 'Itinerario Día a Día',
    day: 'Día',
    included: 'Qué está incluido',
    notIncluded: 'No incluido',
    freeCancel: 'Cancelación gratuita hasta 48 horas antes',
  },

  // Accommodations Page
  accommodations: {
    heroBadge: '🏠 Verificado e inspeccionado',
    heroTitle: 'Alojamientos verificados en todo Kirguistán',
    heroSubtitle: 'Hoteles, yurtas y eco-lodges — seleccionados a mano para calidad, confort y experiencia auténtica',
    marketingTitle: 'Reserva con nosotros — Obtén más por menos',
    marketingDescription: 'Inspeccionamos personalmente cada propiedad. Reserva a través de Mountain Magic Tours y obtén beneficios exclusivos que no encontrarás en Booking.com ni Airbnb. Apoya los negocios locales directamente.',
    perks: {
      discount: '10–15% de descuento sobre tarifas',
      transfer: 'Traslado gratuito desde el aeropuerto Manas',
      support: 'Soporte 24/7 en KG/RU/EN',
      bestPrice: 'Garantía de mejor precio',
    },
    filters: {
      region: 'Región',
      type: 'Tipo',
      all: 'Todos',
      allTypes: 'Todos los tipos',
      hotels: 'Hoteles',
      yurtCamps: 'Campamentos de yurtas',
      guesthouses: 'Casas de huéspedes',
      resorts: 'Resorts',
      ecoLodges: 'Eco lodges',
    },
    propertiesFound: '{count} propiedades encontradas',
    propertyFound: '{count} propiedad encontrada',
    bookSave: 'Reservar y ahorrar',
    noResults: 'No hay propiedades que coincidan con tus filtros.',
  },

  // Expeditions Page
  expeditions: {
    heroBadge: '🏔️ Para verdaderos aventureros',
    heroTitle: 'Expediciones extremas en Kirguistán',
    heroSubtitle: 'Altas cumbres, escaladas técnicas, naturaleza remota — supera tus límites en el corazón de Asia Central',
    sectionTitle: 'Nuestras expediciones',
    sectionDescription: 'Elige tu desafío — desde trekkings moderados hasta ascensos de nivel experto',
    whyTitle: 'Por qué elegir nuestras expediciones',
    whyDescription: 'Combinamos experiencia de montañismo de clase mundial con profundo conocimiento local',
    ctaTitle: '¿Listo para el desafío?',
    ctaDescription: 'Contáctanos para discutir tus objetivos de expedición — crearemos un plan personalizado para tu equipo.',
    ctaButton: 'Contáctanos',
    difficulty: {
      moderate: 'Moderada',
      hard: 'Difícil',
      expert: 'Experto',
    },
    whyItems: {
      guides: { title: 'Guías profesionales', description: 'Montañeros certificados con más de 10 años de experiencia en altitud en las cordilleras de Tien Shan y Pamir.' },
      safety: { title: 'Seguridad ante todo', description: 'Protocolos de seguridad integrales, comunicación satelital, botiquines médicos y planes de evacuación.' },
      groups: { title: 'Grupos pequeños', description: 'Máximo 6–10 participantes por expedición para atención personalizada e impacto ambiental mínimo.' },
      permits: { title: 'Todos los permisos incluidos', description: 'Permisos de zona fronteriza, tarifas de parques nacionales y todo el papeleo gestionado por nuestro equipo.' },
    },
    items: [
      { title: 'Expedición al campo base del Pico Lenin', location: 'Pamir-Alay, Región de Osh', duration: '21 días', description: 'Trek al campo base del Pico Lenin (7.134m), uno de los setesímiles más accesibles del mundo.' },
      { title: 'Ascenso al Khan Tengri', location: 'Tien Shan Central', duration: '18 días', description: 'Intenta la impresionante pirámide de mármol del Khan Tengri (7.010m). Escalada técnica en hielo y nieve.' },
      { title: 'Tien Shan: Helicóptero + Trek', location: 'Glaciar Inylchek', duration: '7 días', description: 'Vuela en helicóptero al corazón del Tien Shan, luego camina por valles glaciares prístinos.' },
      { title: 'Montañismo de varios días en la cordillera Ala-Too', location: 'Kyrgyz Ala-Too, Biskek', duration: '10 días', description: 'Escala múltiples picos en la cordillera Ala-Too justo sobre Biskek.' },
      { title: 'Expedición por la carretera del Pamir', location: 'Osh – Pamir – Dusambé', duration: '14 días', description: 'Recorre la legendaria Carretera del Pamir — la segunda carretera internacional más alta del mundo.' },
    ],
  },

  // About Kyrgyzstan Page
  aboutKyrgyzstan: {
    title: 'Sobre Kirguistán',
    subtitle: 'Datos interesantes sobre uno de los países más espectaculares y acogedores de Asia Central — actualizado para 2026.',
    facts: {
      population: { title: 'Población', description: 'Hogar de aproximadamente 7 millones de personas, una vibrante mezcla de kirguises, uzbekos, rusos y otras etnias que comparten un rico tapiz cultural.' },
      capital: { title: 'Capital', description: 'Biskek — una capital moderna y arbolada al pie de las montañas Tien Shan, que combina arquitectura soviética con bulliciosos bazares y cafés.' },
      leninPeak: { title: 'Pico Lenin — 7.134 m', description: 'Uno de los picos más altos de Asia Central, el Pico Lenin atrae a montañeros de todo el mundo por su cumbre desafiante pero accesible.' },
      issykKul: { title: 'Issyk-Kul nunca se congela', description: 'El segundo lago alpino más grande del mundo nunca se congela a pesar de los duros inviernos. Una maravilla natural a 1.607 m de altitud.' },
      yurtCulture: { title: 'Cultura nómada de la yurta', description: 'Los kirguises han vivido en yurtas de fieltro portátiles durante milenios. Hoy puedes alojarte en campamentos de yurtas y experimentar auténtica hospitalidad nómada.' },
      silkRoad: { title: 'Antigua Ruta de la Seda', description: 'Kirguistán fue un corredor clave de la Gran Ruta de la Seda. Caravansarais, petroglifos y la Torre Burana aún resuenan con siglos de comercio.' },
      plantSpecies: { title: '4.000+ especies de plantas', description: 'Desde bosques de nogales — los más grandes naturales de la Tierra — hasta prados alpinos llenos de flores silvestres. Un punto caliente de biodiversidad.' },
      nomadGames: { title: 'Juegos Nómadas 2026', description: 'Cholpon-Ata acoge los Juegos Mundiales Nómadas en septiembre de 2026 — una épica celebración de deportes tradicionales: juegos ecuestres, caza con águila y lucha.' },
      visaFree: { title: 'Sin visa para 60+ países', description: 'Ciudadanos de más de 60 países disfrutan de entrada sin visa hasta 60 días, haciendo de Kirguistán uno de los destinos más accesibles de Asia Central.' },
      cuisine: { title: 'Cocina legendaria', description: 'Saborea beshbarmak, plov, manti y kumis — leche de yegua fermentada considerada la bebida de los guerreros.' },
    },
  },

  // Nomad Games Detail Page
  nomadGamesPage: {
    back: 'Volver',
    specialEvent: '🔥 Evento Especial',
    title: 'Juegos Mundiales Nómadas 2026 — Cultura nómada épica y competiciones',
    location: 'Biskek → Issyk-Kul',
    duration: '10 días / 9 noches',
    countries: '89+ países',
    dates: '30 de agosto – 8 de septiembre de 2026',
    description: 'Un evento global con 89+ países reunidos en las impresionantes montañas del Tien Shan y a orillas del lago Issyk-Kul. Ceremonias de apertura, competiciones épicas y un festival cultural vivo — más que deportes: actuaciones étnicas, música en vivo, mercados nómadas y gastronomía nacional de toda Asia Central.',
    whoIsItFor: {
      title: '¿Para quién es?',
      items: ['Exploradores culturales', 'Amantes de la aventura', 'Fotógrafos', 'Viajeros individuales y parejas'],
    },
    whyItMatters: {
      title: 'Por qué es importante',
      description: 'Más que deportes — los Juegos Mundiales Nómadas son una celebración del patrimonio vivo. Espera actuaciones étnicas, música en vivo de toda Asia Central, mercados con artesanías y gastronomía nacional auténtica. Una inmersión cultural única en la vida.',
    },
    programHighlights: {
      title: 'Eventos destacados',
      items: ['Deportes ecuestres (Kok Boru, carreras)', 'Tiro con arco y tiro montado', 'Lucha tradicional (Alysh, Kurash)', 'Cetrería y caza con águila (Burkut Saluu)', 'Carreras de perros Taigan', 'Tradiciones nómadas y Aldea Étnica'],
    },
    dayByDay: {
      title: 'Día a día',
      days: [
        { date: '30 de agosto', title: 'Día 1 — Llegada a Biskek', description: 'Equipos y oficiales llegan al Aeropuerto Internacional Manas. Traslados premium a hoteles cuidadosamente seleccionados. 💡 Da un paseo nocturno por el centro para sentir la vibra de Kirguistán.' },
        { date: '31 de agosto', title: 'Día 2 — Gran ceremonia de apertura', description: 'Un espectáculo dinámico celebrando el patrimonio cultural de todas las naciones participantes: música, danza y trajes tradicionales. Nuestros guías compartirán historias fascinantes.' },
        { date: '1 de septiembre', title: 'Día 3 — Traslado a Issyk-Kul', description: 'Traslado premium. Paradas escénicas: Torre Burana, petroglifos, cañones pintorescos — perfecto para fotos. Check-in en resort junto al lago o lodge estilo yurta.' },
        { date: '2 de septiembre', title: 'Día 4 — Comienzan deportes y cultura', description: 'Deportes: Kok Boru, Alysh, Toguz Korgool, Mangala. Cultura: Apertura del "Universo de los Nómadas" y Conferencia Científica. Aldea Étnica: juegos demostrativos y actuaciones culturales.' },
        { date: '3 de septiembre', title: 'Día 5 — Actividades del festival', description: 'Deportes: Kok Boru, Kazakh Kuresi, Kyrgyz Kurosh, Armwrestling. Aldea Étnica: Ordo, tira y afloja, caza con águila, carreras Taigan, tiro con arco. 💡 ¡No te pierdas los shows de águilas!' },
        { date: '4 de septiembre', title: 'Día 6 — Programa nocturna', description: 'Deportes: Kok Boru, Kurash, Sambo, Toguz Korgool. Cultura: Continuación del festival "Universo de los Nómadas". Aldea Étnica: Ordo, tira y afloja, tiro con arco montado.' },
        { date: '5 de septiembre', title: 'Día 7 — Eventos finales', description: 'Deportes: Kok Boru, Er Enish, carreras planas, Kunan Chabysh. Cultura: Clausura del "Universo de los Nómadas". Aldea Étnica: concursos de fuerza y shows interactivos. 💡 ¡El día más dinámico!' },
        { date: '6 de septiembre', title: 'Día 8 — Ceremonia de clausura', description: 'Deportes: Kok Boru, Er Enish, Jorgo Salysh, carreras finales. "La Gran Lucha de los Nómadas". Clausura oficial de los VI Juegos Mundiales Nómadas. Celebración y fotos con participantes.' },
        { date: '7 de septiembre', title: 'Día 9 — Regreso a Biskek', description: 'Traslado premium con parada para almuerzo. Tiempo libre en Biskek. Cena de despedida con cocina local y recuerdos del viaje.' },
        { date: '8 de septiembre', title: 'Día 10 — Partida', description: '✈ Vuelo a casa. Traslado al aeropuerto. Fin del tour.' },
      ],
    },
    practical: {
      title: 'Información práctica',
      includedTitle: 'Qué está incluido',
      included: ['Alojamiento (3–4★ o similar)', 'Desayunos / almuerzos / cenas diarios', 'Todos los traslados premium', 'Guía de habla inglesa y coordinación', 'Acceso a eventos y zonas del festival', 'Agua durante todo el tour'],
      notIncludedTitle: 'No incluido',
      notIncluded: ['Vuelos internacionales', 'Tasas de visa', 'Gastos personales'],
      accommodationTitle: 'Alojamiento',
      accommodationDescription: 'Biskek: hotel urbano (3–4★) · Issyk-Kul: resort junto al lago o lodge estilo yurta · Autobuses / minivans premium · Formato de grupo pequeño',
    },
    packing: {
      title: 'Clima y qué llevar',
      subtitle: 'Días cálidos, noches frescas cerca del lago Issyk-Kul.',
      items: ['Zapatos cómodos para caminar (los recintos son grandes)', 'Chaqueta ligera o cortavientos (noches frescas junto al lago)', 'Protector solar y gafas (sol fuerte de montaña)', 'Sombrero o pañuelo (protección solar y respeto cultural)', 'Power bank / batería extra (días largos de festival)', 'Botella de agua reutilizable', 'Mochila pequeña o bolso cruzado'],
    },
    optional: {
      title: 'Experiencias opcionales',
      items: ['Asientos VIP en competiciones clave', 'Taller de caza con águila y tiro con arco', 'Asistencia fotográfica profesional', 'Extensiones post-tour (Karakol / Son-Kul)'],
    },
    schedule: {
      title: 'Horario y estado del tour',
      startDate: 'Fecha de inicio',
      endDate: 'Fecha de fin',
      status: 'Estado',
      currentBookings: 'Reservas actuales',
    },
    booking: {
      priceOnRequest: 'Precio a consultar',
      priceDescription: 'Cada viaje se adapta individualmente según el tamaño del grupo, la categoría de alojamiento y las experiencias seleccionadas.',
      dateRange: '30 ago – 8 sep 2026',
      onceInLifetime: 'Un evento único en la vida',
      bookNow: 'Reservar ahora',
      freeCancel: 'Cancelación gratuita hasta 48 horas antes',
    },
  },

  // Booking Form
  bookingForm: {
    title: 'Reservar: {tour}',
    name: 'Nombre *',
    email: 'Email *',
    phone: 'Teléfono / WhatsApp *',
    phonePlaceholder: '+996...',
    dates: 'Fechas preferidas',
    datesPlaceholder: 'ej. 15–20 de junio de 2026',
    people: 'Número de personas',
    message: 'Mensaje',
    messagePlaceholder: 'Solicitudes especiales...',
    submit: 'Enviar solicitud de reserva',
    submitting: 'Enviando...',
    successTitle: '¡Solicitud enviada!',
    successDescription: 'Nos pondremos en contacto en 24 horas.',
    note: 'Tu solicitud se enviará por email y WhatsApp. Responderemos en 24 horas.',
  },
};